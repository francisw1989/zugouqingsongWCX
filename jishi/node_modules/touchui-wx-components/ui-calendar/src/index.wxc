    ...
    <swiper class="calendar-wrap"
      ...
      >
        ...
        >
          ...
          >
            <view 
            class="calendar-day-cell {{disablePastDays && dayItem.isPastDay ? 'is-past-days' : ''}} {{dayItem.isHoliday && dayItem.isThisMonth ? 'is-holiday' : ''}} {{dayItem.isThisMonth ? 'is-this-month' : 'not-this-month'}} {{dayItem.yes ? 'is-selected' : ''}} {{dayItem.isRangeLeft ? 'is-selected-left' : ''}} {{dayItem.isRangeRight ? 'is-selected-right' : ''}} {{dayItem.isInRange? 'is-selected-in-range' : ''}}"
  ...
  methods: {
    ...
    getMonthArray (date, monthIndex) {
      ...
      for (let i = 0; i < date.thisMonthDays.length; i++) {
        ...
        obj.dayIndex = count
        obj.value = `${obj.year}/${obj.month < 10 ? '0' : ''}${obj.month}/${obj.day < 10 ? '0' : ''}${obj.day}`
        this.data.workingdays.length && this.data.workingdays[i] && (obj.yes = this.data.workingdays[i].yes);
        if ([0, 6].indexOf((moment(obj.value).weekday())) !== -1) {
          obj.isHoliday = true
        }
